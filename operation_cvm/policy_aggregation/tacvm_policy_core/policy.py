...
def validate_proposal(
    ...
    for section in (
        ...
        if section not in proposal:
            _fail("INVALID_PROPOSAL", f"Missing policy section {section}")


def proposal_digest(
    proposal: Mapping[str, Any], *, already_normalized: bool = False
) -> str:
    normalized = proposal if already_normalized else normalize_proposal(proposal)
    constraints = {
...
def join_policy_bodies(
    proposals: Sequence[Mapping[str, Any]], *, already_normalized: bool = False
) -> Dict[str, Any]:
    normalized = (
        list(proposals)
        if already_normalized
        else [normalize_proposal(proposal) for proposal in proposals]
    )
    return {
...
def build_candidate(
    ...
    for participant_id in ordered_ids:
        raw = proposals_by_participant[participant_id]
        validate_proposal(raw, context, expected_ids)
        try:
    ...
    candidate = {
        ...
        "inputs": inputs,
        "policy": join_policy_bodies(proposals, already_normalized=True),
    }
    return {
        "candidate": candidate,
...
def build_candidate_from_accepted(
    context: Mapping[str, Any],
    participant_order: Sequence[str],
    accepted: Mapping[str, Mapping[str, Any]],
) -> Dict[str, Any]:
    """Build a candidate from proposals already verified at submit time.

    ``accepted[participant_id]`` must contain normalized ``proposal`` and
    ``proposal_digest`` produced during ``submit_proposal``. No second
    normalize/validate/digest pass is performed.
    """
    ...
    inputs = []
    for participant_id in ordered_ids:
        envelope = accepted[participant_id]
        proposal = envelope["proposal"]
        digest_value = envelope["proposal_digest"]
        if not isinstance(proposal, Mapping) or not isinstance(digest_value, str):
            _fail(
                "INVALID_PROPOSAL",
                f"Accepted envelope for {participant_id} is incomplete",
            )
        if proposal.get("author", {}).get("participant_id") != participant_id:
            _fail(
                "PROPOSAL_AUTHOR_MISMATCH",
                f"Proposal stored for {participant_id} has another author",
            )
...
def verify_candidate_against_proposal(
    ...
    try:
        own_normalized = normalize_proposal(proposal)
        candidate_as_proposal = normalize_proposal(_candidate_as_proposal(candidate))
        restricted = join_policy_bodies(
            [own_normalized, candidate_as_proposal],
